import { copySync } from "https://deno.land/std@0.191.0/fs/copy.ts";
import { emptyDirSync } from "https://deno.land/std@0.191.0/fs/empty_dir.ts";
import { Plugin } from "https://deno.land/x/esbuild@v0.18.1/mod.js";
import { ensureNestedFolderExists } from "./filesystem.ts";
import { assert } from "https://deno.land/std@0.191.0/_util/asserts.ts";
...
export function autoGeneratedTemplate(opts: TemplateConfig, outdir: string, id: string) {
    ...
    assert(fallbackName);
    Deno.writeTextFileSync(`${outdir}/${id}.html`, opts.defaultTemplate?.(fallbackName) ?? `<link rel="stylesheet" href="${fallbackName}.css"><script src="${fallbackName}.js" type="module"></script>`, { create: true });
}
